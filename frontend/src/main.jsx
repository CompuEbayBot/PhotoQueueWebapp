...
async function api(action, idToken, payload = {}) {
  ...
  try {
    ...
    throw new Error(`Backend returned a non-JSON response: ${text.slice(0, 180)}`);
  }
...
function App() {
  const [idToken, setIdToken] = useState(sessionStorage.getItem('photoQueueIdToken') || '');
  ...
  const filtered = useMemo(() => {
    ...
    return queue.filter((item) => {
      ...
      if (!q) return true;
      return [item.model, item.grade, item.customCode, item.category, item.ghost]
        .filter(Boolean)
  ...
  async function uploadPhotos(item, files) {
    ...
    if (tooLarge) return setError(`${tooLarge.name} is over the 8 MB starter limit.`);
    ...
    try {
      ...
      setNotice(`Uploaded ${result.uploadedCount} photo(s) for ${item.customCode || item.model}.`);
      await loadQueue();
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    return (
      <main className="login-shell">
        <section className="login-card">
          ...
          <h1>Photo Queue</h1>
          <p>Sign in with an approved Google account to view and upload product photos.</p>
          {!CLIENT_ID && <div className="alert error">Missing VITE_GOOGLE_CLIENT_ID</div>}
  ...
  return (
    <main className="app-shell">
      ...
      <section className="toolbar">
        <input
          type="search"
          placeholder="Search model, grade, code, ghost note..."
          value={search}
      ...
      <section className="queue-card">
        <div className="queue-heading">
          <div>
            <h2>{selectedCategory === 'All' ? 'All pending photos' : selectedCategory}</h2>
            <p>{filtered.length} item{filtered.length === 1 ? '' : 's'} shown</p>
          </div>
        </div>
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.rowKey}>
                  ...
                  <td><span className={`grade grade-${String(item.grade).toLowerCase()}`}>{item.grade}</span></td>
                  <td className="mono">{item.customCode}</td>
                  <td>{item.ghost || '—'}</td>
                  <td>
                    <label className={`upload-button ${uploadingKey === item.rowKey ? 'disabled' : ''}`}>
                      {uploadingKey === item.rowKey ? 'Uploading…' : 'Choose photos'}
                      <input
                        type="file"
                        accept="image/*"
                        multiple
              ...
              {!filtered.length && !loading && (
                <tr>
                  <td colSpan="6" className="empty">Nothing pending in this view 🎉</td>
                </tr>
              )}
            </tbody>
          </table>
